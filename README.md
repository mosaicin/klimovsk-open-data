...
## Скрининг признаков картельного сговора

Новый нейтральный отчёт [cartel-screening-ocd.md](cartel-screening-ocd.md) сопоставляет доступную выборку закупок ОЦД с публичными признаками риска ФАС. Отчёт не устанавливает картель: полная выгрузка 232 контрактов и технические материалы торгов не были доступны.

## Скрипт проверки контрактов и аффилированности

Добавлены [Python-скрипт проверки статусов контрактов](check_ocd_contracts.py) и [отчёт по индикаторам аффилированности с заказчиками](ocd-affiliation-risk.md). Скрипт требует API-ключ стороннего машиночитаемого сервиса или локальный JSON-экспорт; он не подменяет ручную проверку карточек ЕИС и не делает юридических выводов по совпадениям.
